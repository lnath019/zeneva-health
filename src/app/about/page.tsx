...
function HowWeVerifySection() {
  ...
  const title = 'What it takes to get listed on Zeniva';
    ...
    <section className="bg-tertiary py-20">
      <div className="mx-auto w-[95%] max-w-none">
        <div ref={headerRef} className="mx-auto max-w-2xl text-center">
          ...
          <p
            className="mt-3 text-sm sm:text-base leading-relaxed text-slate-500 transition-all duration-700 ease-out"
            style={{
              opacity: doneTyping ? 1 : 0,
              transform: doneTyping ? 'translateY(0)' : 'translateY(10px)',
            }}
          >
            Every provider on our platform goes through the same three checks before patients
        ...
        <div className="relative mt-14 grid grid-cols-1 gap-8 md:grid-cols-3">
          ...
          {VERIFICATION_STEPS.map((item, index) => (
            <div
              key={item.step}
              className="flex flex-col items-center rounded-2xl border border-slate-100 bg-white p-6 text-center shadow-sm transition-all duration-500 ease-out hover:-translate-y-0.5 hover:shadow-md"
              style={{
                opacity: doneTyping ? 1 : 0,
                transform: doneTyping ? 'translateY(0)' : 'translateY(16px)',
                transitionDelay: `${150 + index * 130}ms`,
              }}
            >
              <span
                className={`inline-flex h-14 w-14 items-center justify-center rounded-full text-lg font-extrabold text-white shadow-md ring-4 ${
                  index % 2 === 0 ? 'bg-primary ring-primary/10' : 'bg-secondary ring-secondary/10'
                }`}
              >
                {item.step}
              </span>
              <div className="mt-3 flex h-9 w-9 items-center justify-center rounded-lg bg-primary-light text-primary">
                <Icon name={item.icon} className="w-4 h-4" />
              </div>
