        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {FEATURED_SERVICES.map((service) => (
            <Link
              ...
              href={service.href}
              className={cn(
                'group flex items-start gap-4 bg-white p-6 rounded-xl border border-slate-100 border-l-4 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-200',
                service.accent,
                service.glow
              )}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {HOW_IT_WORKS.map((item, index) => (
            ...
            >
              <span
                className={cn(
                  'inline-flex shrink-0 items-center justify-center w-14 h-14 rounded-full text-white text-xl font-extrabold shadow-md ring-4',
