  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
        <h2 className="text-xl font-bold text-slate-800">Lab Bookings</h2>
        <p className="text-slate-500 text-xs mt-1">Review patients booked against your test slots.</p>
      </div>
      ...
      {isSlotsLoading ? (
        ...
        <div className="bg-white border border-slate-100 rounded-xl p-12 text-center text-slate-500 font-medium">
          You have no slots yet. Create one from &quot;Manage Lab Slots&quot; to start receiving bookings.
        </div>
      ) : (
        <>
            ...
            >
              {slots.map((slot: LabSlot) => (
                <option key={slot.id} value={slot.id}>
                  {slot.slotDate} &bull; {slot.startTime?.slice(0, 5)}-{slot.endTime?.slice(0, 5)} &bull; max {slot.maxTokens}
                </option>
              ))}
            </select>
          </div>
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {appointments
                    .slice()
                    .sort((a: LabAppointment, b: LabAppointment) => a.tokenNumber - b.tokenNumber)
                    .map((appt: LabAppointment) => (
                            ...
                            <span
                              className={`w-2 h-2 rounded-full ${
                                appt.status === 'confirmed' || appt.status === 'completed'
                                  ? 'bg-emerald-700'
                                  : appt.status === 'cancelled' || appt.status === 'needs_reschedule'
                                  ? 'bg-rose-700'
                                  : 'bg-amber-700'
                              }`}
                            />
