...
export function Sidebar() {
  ...
  const navItems: NavItem[] = [
    ...
    },
    {
      ...
      roles: ['patient'],
      path: '/dashboard?tab=book-appointment',
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    },
    {
      ...
      roles: ['patient'],
      path: '/dashboard?tab=patient-appointments',
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    },
    {
      ...
      label: 'Manage Slots',
      roles: ['doctor'],
      path: '/dashboard?tab=manage-slots',
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  const isLinkActive = (item: NavItem) => {
    if (item.path.startsWith('/dashboard/hospitals')) {
      return pathname.startsWith('/dashboard/hospitals');
    }
    if (item.path.startsWith('/dashboard/ambulances')) {
      return pathname.startsWith('/dashboard/ambulances');
    }
    if (item.id === 'overview') {
      return pathname === '/dashboard' && !searchParams.get('tab');
    }
    return pathname === '/dashboard' && searchParams.get('tab') === item.id;
  };
